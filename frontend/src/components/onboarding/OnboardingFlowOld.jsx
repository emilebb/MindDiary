    ...
    {[
      {
        ...
        description: 'Tengo muchas ideas sueltas y necesito darles estructura',
        icon: '📋',
      ...
      },
      {
        ...
        description: 'Quiero desarrollar un proyecto desde cero',
        icon: '🎨',
        color: 'from-purple-500 to-pink-600'
      },
      {
        id: 'overcome-block',
        title: 'Superar un bloqueo creativo',
        description: 'Siento que estoy atascado y necesito inspiración',
        icon: '🧩',
        color: 'from-green-500 to-emerald-600'
      },
      {
        ...
        description: 'Quiero descubrir nuevas direcciones creativas',
        icon: '🔍',
        color: 'from-orange-500 to-red-600'
      }
    ].map((goal) => (
      <motion.button
        ...
        className={`
          ...
          ${preferences.primaryGoal === goal.id
            ? `border-purple-500 bg-gradient-to-r ${goal.color} text-white`
...
const StyleStep = ({ preferences, setPreferences }) => (
  <div className="space-y-6">
    <div>
      <h3 className="text-xl font-semibold text-white mb-4">¿Cómo te sientes más cómodo creando?</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          {
            id: 'visual',
            title: 'Visual',
            description: 'Mapas mentales, colores, imágenes',
            icon: '👁️'
          },
          {
            id: 'textual',
            title: 'Textual',
            description: 'Escritura, notas, palabras',
            icon: '✍️'
          },
          {
            id: 'kinesthetic',
            title: 'Kinestésico',
            description: 'Dibujos, movimiento, hands-on',
            icon: '🙌'
          },
          {
            id: 'mixed',
            title: 'Mixto',
            description: 'Combinación de todo',
            icon: '🌈'
          }
        ].map((style) => (
          <motion.button
    ...
    <div>
      <h3 className="text-xl font-semibold text-white mb-4">¿Cuál es tu experiencia con herramientas creativas?</h3>
      <div className="flex gap-4">
        {[
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => setPreferences({ ...preferences, experience: level.id })}
            className={`
  ...
  const actions = [
    {
      id: 'capture',
      title: 'Capturar una idea',
      description: 'Guarda ese pensamiento que no quieres olvidar',
      icon: '💡',
      action: () => onComplete({ ...preferences, firstAction: 'capture' })
    },
    {
      id: 'mindmap',
      title: 'Crear un mapa mental',
      description: 'Visualiza tus ideas y sus conexiones',
      icon: '🗺️',
      action: () => onComplete({ ...preferences, firstAction: 'mindmap' })
    },
    {
      id: 'flow',
      title: 'Entrar en modo flujo',
      description: 'Un espacio sin distracciones para crear libremente',
      icon: '🌊',
      action: () => onComplete({ ...preferences, firstAction: 'flow' })
    }
  ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {actions.map((action, index) => (
        <motion.button
          ...
          className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 p-8 hover:border-purple-500 transition-all duration-300"
        >
